...
const app = new Elysia({ adapter: CloudflareAdapter })
  ...
  .post('/api/auth/verify-identity', async ({ body, set }) => {
    ...
    try {
      ...
      const humanSearchParams = new URLSearchParams({ filter: `wallet_address = "${walletAddress}"` })
      const humanSearchRes = await fetch(`${PB_URL}/api/collections/humans/records?${humanSearchParams}`)
      ...
      if (humanSearchData.items?.length > 0) {
        ...
        if (adminAuthForHuman.token) {
          ...
          if (updateHumanRes.ok) {
            human = await updateHumanRes.json()
          }
        }
      } else {
        // Create new human
